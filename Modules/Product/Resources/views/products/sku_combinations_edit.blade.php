...
@foreach ($combinations as $key => $combination)
	...
	@if(strlen($str) > 0)
			<tr class="variant text-center">
				...
				<td class="pt-25">
					<input  class="primary_input_field mt-30 selling_price" type="number" name="selling_price_sku[]" value="{{ ($query_1) ? @$query_1->selling_price : "0" }}" min="0" step="{{step_decimal()}}" class="form-control" required>
				</td>
				@if(!isModuleActive('MultiVendor'))
					...
					<div class="primary_input mb-10">
						...
						<div class="primary_file_uploader mt-50" data-toggle="amazuploader" data-multiple="false" data-type="image" data-name="variant_image_{{$imgIncrement}}">
						  ...
						  <button class="" type="button">
								...
								<input type="hidden" class="selected_files" value="{{@$query_1->variant_image_media->media_id}}">
							</button>
					   </div>
